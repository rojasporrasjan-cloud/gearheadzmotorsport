...
export function buildEventPreviewCard(ev, index = 0) {
  ...
  const btn      = ev.instagramUrl
    ? `<a href="${ev.instagramUrl}" target="_blank" rel="noopener" class="btn-ticket learn-more">LEARN MORE ↗</a>`
    : '';
  const timeStr  = ev.timeStart ? `${ev.timeStart} – ${ev.timeEnd}` : 'Details dropping soon';

  return `
    <div class="ev-card reveal ${delay}">
      <div class="ev-card-img" style="background-image:url('${ev.img}');background-size:cover;background-position:center">
        <div class="ev-card-date-tag"${isSoon ? ' style="background:#222"' : ''}>
          <span class="ev-date-month">${ev.month}</span>
          <span class="ev-date-day"${isSoon ? ' style="font-size:1.3rem;line-height:1.3"' : ''}>${ev.day}</span>
        </div>
        <span class="ev-card-status ${ev.status}">${STATUS_LABEL[ev.status] || ''}</span>
      </div>
      <div class="ev-card-body">
        <div class="ev-card-name">${ev.name}</div>
        <div class="ev-card-loc">📍 ${ev.location}</div>
...
export function buildEventFullCard(ev, index = 0) {
  ...
  const timeStr = ev.timeStart
    ? `${ev.month} ${ev.day}${ev.year ? ', ' + ev.year : ''} · ${ev.timeStart} – ${ev.timeEnd}`
    : 'Date & location TBA';
  const locEl   = ev.mapsLink
    ? `<div class="ev-full-loc">📍 <a href="${ev.mapsLink}" target="_blank" rel="noopener">${ev.location}</a></div>`
    : `<div class="ev-full-loc">📍 ${ev.location}</div>`;
  const mapEl   = ev.mapsEmbed
    ? `<div class="ev-full-map"><iframe src="${ev.mapsEmbed}" width="100%" height="220" style="border:0;border-radius:8px;margin-top:.75rem" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>`
    : '';
  const instagramEl = ev.instagramUrl
    ? `<div class="ev-full-instagram">
         <blockquote class="instagram-media" data-instgrm-captioned data-instgrm-permalink="${ev.instagramUrl}" data-instgrm-version="14" style="background: #FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; width: 100%;">
           <div style="padding:16px;">
             <a href="${ev.instagramUrl}" style="color:#c9c8cd; text-decoration:none;" target="_blank">Ver publicación en Instagram</a>
           </div>
         </blockquote>
       </div>`
    : '';
  ...
  return `
    <div class="ev-full-card reveal ${delay}">
      <div class="ev-full-img" style="background-image:url('${ev.img}');background-size:cover;background-position:center">
        <div class="ev-full-date-tag"${isSoon ? ' style="background:#222"' : ''}>
          <span class="ev-full-date-month">${ev.month}</span>
          <span class="ev-full-date-day"${isSoon ? ' style="font-size:1.5rem;line-height:1.3"' : ''}>${ev.day}</span>
        </div>
        <span class="ev-full-status ${ev.status}">${STATUS_LABEL[ev.status] || ''}</span>
      </div>
      <div class="ev-full-body">
        <div class="ev-full-name">${ev.name}</div>
        <div class="ev-full-time">${timeStr}</div>
        ${locEl}
        <p class="ev-full-desc">${ev.description}</p>
        ${detailsBtn}
        ${collapsibleContainer}
        ${btn}
      </div>
